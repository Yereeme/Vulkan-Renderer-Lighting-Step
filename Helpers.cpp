...
Helpers::AllocatedBuffer Helpers::create_buffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, MapFlag map) {
	...
	// bind memory
	VK(vkBindBufferMemory(rtg.device, buffer.handle, buffer.allocation.handle,
...
void Helpers::transfer_to_buffer(void const *data, size_t size, AllocatedBuffer &target) {
	//NOTE: could let this stick around and use it for all uploads, but this function 
	//isn't for permanent transfers anyway:
	...
	{
		VK(vkResetCommandBuffer(transfer_command_buffer, 0)); //reset command buffer to initial

		VkCommandBufferBeginInfo begin_info{//struct specifying a command buffer begin 
			//operation
		...
		vkCmdCopyBuffer(transfer_command_buffer, transfer_src.handle, target.handle, 1,
			&copy_region); //copy data between buffer region
	...
	// run command buffer:
	VkSubmitInfo submit_info{ //struct specifying a queue submit operation
	...
	//wait for command buffer to finish (WHEN GRAPHICS QUEUE IS IDLE)
...
//CREATE AND DESTROY COMMAND POOL AND BUFFERS
void Helpers::create() {
	...
	if (rtg.configuration.debug) {
		...
		for (uint32_t i = 0; i < memory_properties.memoryTypeCount; ++i) {
			VkMemoryType const& type = memory_properties.memoryTypes[i];
			std::cout << " [" << i << "] heap " << type.heapIndex << ", flags: " << 
				string_VkMemoryPropertyFlags(type.propertyFlags) << '\n';
		}
		std::cout << "Memory heaps:\n";
		for (uint32_t i = 0; i < memory_properties.memoryHeapCount; ++i) {
			VkMemoryHeap const& heap = memory_properties.memoryHeaps[i];
			std::cout << " [" << i << "] heap " << heap.size << " bytes, flags: " <<
				string_VkMemoryHeapFlags(heap.flags) << '\n';
		}
		std::cout.flush();
	}
}
...
void Helpers::destroy() {
	...
	if (transfer_command_buffer != VK_NULL_HANDLE) {
		vkFreeCommandBuffers(rtg.device, transfer_command_pool, 1, 
			&transfer_command_buffer);
